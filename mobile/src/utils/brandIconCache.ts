...
const ICON_PATH = '/static/theme-brand-icon-transparent.png'
...
/** 登录页 Logo 展示地址，命中缓存时为 data URL，可即时渲染 */
...
export function warmBrandIconCache(): Promise<string> {
  ...
  warming = new Promise<string>((resolve) => {
    // #ifdef H5
    fetch(ICON_PATH)
      .then((res) => {
      ...
      .then((dataUrl) => {
        ...
        try {
          uni.setStorageSync(STORAGE_KEY, dataUrl)
        } catch {
          /* ignore */
        }
        resolve(dataUrl)
      })
      .catch(() => resolve(brandIconSrc.value))
      .finally(() => {
        warming = null
      })
    // #endif
