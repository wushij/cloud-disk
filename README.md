...
为了与 PC 端的极简石墨商务（slate）视觉语言保持高度统一，近期对 `mobile` 移动端进行了深度的视觉重构和交互优化：

1. **统一石墨黑卡片设计系统**：
   - 核心页面（云盘、团队、分享、回收站、个人中心）头部重构为浮动圆角卡片，应用相同的石墨黑渐变（`linear-gradient`）和径向漫反射光效（`radial-gradient`）。
   - 图标遵循“无框化、无阴影”的极简主义设计，去除所有繁杂的外围容器，让纯色矢量 SVG 直接浮动展示。

2. **“我的”个人中心（`/profile`）全面升级**：
   - **名片排版**：重构为“左侧圆形头像，右侧单行昵称 · 用户名”的流式布局，并在卡片内部水平居中，提升对称美感。头像区新增悬浮的相机编辑小图标。
   - **容量展示**：消除了存储卡片与顶部个人名片的遮挡与重叠（修改为标准 `24rpx` 正向边距），进度条加高至 `16rpx` 并使用金属色系渐变填充。
   - **设置分组**：采用 iOS 分组卡片风格收纳，包括“团队空间”、“我的分享”、“回收站”、“清除缓存（模拟清理并气泡提示）”和“关于我们（关于弹窗）”，各子项采用矢量 SVG 与超浅底部边框区隔。
   - **操作优化**：退出登录卡片重构为极具质感的红色危险渐变胶囊按钮，加入点击微缩放动画以增强触碰反馈。
   - **去冗余化**：移除了作为主导航页不必要的返回按钮以及状态标签，简化信息层级。

3. **关键交互与体验修复**：
   - **拦截点击冒泡**：彻底修复了文件列表条目右侧更多操作按钮（`...`）在部分移动端浏览器上会冒泡触发文件打开的逻辑冲突。通过在 `FileListItem.vue` 按钮组件上拦截并阻断 `click` 和 `touchstart` 事件实现完美拦截。
...
│   ├── src/pages/              # 登录、云盘、分享、回收站、个人中心
│   ├── src/stores/             # 与 PC 共用 API 的 Pinia Store
